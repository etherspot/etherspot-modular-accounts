...
library ExecutionLib {
     ...
     */
    function decodeBatch(bytes calldata _executionCalldata)
        internal
        pure
        returns (Execution[] calldata executionBatch)
    {
        ...
        assembly {
            let u := calldataload(_executionCalldata.offset)
            let s := add(_executionCalldata.offset, u)
            let e := sub(add(_executionCalldata.offset, _executionCalldata.length), 0x20)
    ...
    function encodeBatch(Execution[] memory _executions) internal pure returns (bytes memory callData) {
        callData = abi.encode(_executions);
    }
    ...
    function encodeSingle(address _target, uint256 _value, bytes memory _callData)
        internal
        pure
        returns (bytes memory userOpCalldata)
    {
        userOpCalldata = abi.encodePacked(_target, _value, _callData);
    }
}
