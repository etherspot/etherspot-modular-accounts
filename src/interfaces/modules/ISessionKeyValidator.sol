...
import {ParamCondition, Permission, SessionData} from "../../types/Structs.sol";

/// @title ISessionKeyValidator
/// @author @cryptonoyaiba | Etherspot
/// @notice Interface for a validator module that handles session key functionality
/// @dev Extends the standard IValidator interface with session key specific methods and events
interface ISessionKeyValidator is IValidator {
    ...
    function enableSessionKey(SessionData memory _sessionData, Permission[] memory _permissions) external;
    ...
    function getUsesLeft(address _sessionKey, uint256 _permissionIndex) external view returns (uint256);
