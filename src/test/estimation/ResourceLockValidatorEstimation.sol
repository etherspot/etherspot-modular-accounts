...
// Estimation contract for ResourceLockValidator
...
contract ResourceLockValidator is IResourceLockValidator {
    ...
    function validateUserOp(PackedUserOperation calldata userOp, bytes32 userOpHash)
        ...
        returns (uint256)
    {
        console2.log("********** ResourceLockValidator::validateUserOp() **********");
        console2.log("msg.sender:", msg.sender);
        if (credibleAccountModule == address(0)) revert RLV_CredibleAccountModuleNotSet();
        bytes calldata signature = userOp.signature;
        console2.log("UserOperation.signature (complete):");
        ...
        console2.log("UserOperation.signature (sig only):");
        console2.logBytes(ecdsaSignature);
        bytes32 root = bytes32(signature[65:97]); // 32 bytes
        console2.log("UserOperation.signature (root only):");
        ...
        if (signature.length > 97) {
            ...
            for (uint256 i; i < proofCount; ++i) {
                ...
                console2.log("UserOperation.signature (proof): ", i);
