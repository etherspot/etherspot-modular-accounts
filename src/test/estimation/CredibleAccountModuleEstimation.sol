...
// Estimation contract for CredibleAccountModule
// Used only in testing/estimation flow
...
contract CredibleAccountModule is ICredibleAccountModule, AccessControlEnumerable, ReentrancyGuard {
    ...
    function enableSessionKey(bytes calldata _resourceLock) external {
        ...
        console2.log("********** CredibleAccountModule::enableSessionKey() **********");
        console2.log("msg.sender:", msg.sender);
        console2.log("********** Decoded ResourceLock **********");
        console2.log("ResourceLock.smartWallet: ", rl.smartWallet);
        console2.log("ResourceLock.sessionKey: ", rl.sessionKey);
        console2.log("ResourceLock.solver: ", rl.solver);
        console2.log("ResourceLock.chainId: ", rl.chainId);
        console2.log("ResourceLock.validAfter: ", rl.validAfter);
        console2.log("ResourceLock.validUntil: ", rl.validUntil);
        console2.log("ResourceLock.bidHash:");
        console2.logBytes32(rl.bidHash);
        ...
        for (uint256 i; i < rl.tokenData.length; ++i) {
            ...
            console2.log("ResourceLock::LockedToken::Token: ", rl.tokenData[i].token);
            console2.log("ResourceLock::LockedToken::Amount: ", rl.tokenData[i].amount);
        }
    ...
    function validateUserOp(PackedUserOperation calldata userOp, bytes32 userOpHash)
        ...
        returns (uint256)
    {
        console2.log("********** CredibleAccountModule::validateUserOp() **********");
        console2.log("msg.sender: ", msg.sender);
        console2.log("UserOperation.sender: ", userOp.sender);
        console2.log(
            "Is CredibleAccountModule initialized for wallet: ", moduleInitialized[msg.sender].validatorInitialized
        );
    ...
    function _validateSelector(bytes4 _selector) internal pure returns (bytes4) {
        console2.log(
            "CredibleAccountModule::_validateSelector: is this.claim.selector: ", _selector == this.claim.selector
        );
        console2.log(
            "CredibleAccountModule::_validateSelector: is ERC20.approve.selector: ",
            _selector == IERC20.approve.selector
        );
    ...
    function _validateApproveCall(bytes calldata callData) internal view returns (bool) {
        ...
        console2.log("CredibleAccountModule::_validateApproveCall::spender: ", spender);
        console2.log("CredibleAccountModule::_validateApproveCall::amount: ", amount);
