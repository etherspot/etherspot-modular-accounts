...
import {ExecutionValidation, ParamCondition, Permission, SessionData} from "../../../../src/types/Structs.sol";
...
contract SessionKeyValidator_Fuzz_Test is TestUtils {
    ...
    function testFuzz_enableSessionKey(
        ...
        assertEq(data.validAfter, _validAfter, "ValidAfter should match the set value");
        assertEq(data.validUntil, _validUntil, "ValidUntil should match the set value");
        ...
        assertEq(permissions.length, perms.length, "Permission length should match");
        for (uint256 i; i < perms.length; ++i) {
            ...
            assertEq(perm.selector, expectedPerm.selector, "Selector should match");
            assertEq(perm.payableLimit, expectedPerm.payableLimit, "PayableLimit should match");
            assertEq(perm.uses, expectedPerm.uses, "Uses should match the set value");
            for (uint256 j; j < expectedPerm.paramConditions.length; ++j) {
                ParamCondition memory condition = permissions[i].paramConditions[j];
                ParamCondition memory expectedCondition = expectedPerm.paramConditions[j];
                assertEq(condition.offset, expectedCondition.offset, "Offset should match");
                assertEq(uint8(condition.rule), uint8(expectedCondition.rule), "Rule should match");
                assertEq(condition.value, expectedCondition.value, "Value should match");
            }
        }
        // Verify associated session keys
        address[] memory associatedKeys = SESSION_KEY_VALIDATOR.getSessionKeysByWallet();
        assertEq(associatedKeys.length, 1, "Should have one associated session key");
        assertEq(associatedKeys[0], sessionKey.pub, "Associated session key should match the enabled key");
        vm.stopPrank();
    }

    function testFuzz_toggleSessionKeyPause(uint8 _toggleCount) public validatorInstalled {
        ...
        for (uint8 i; i < _toggleCount; ++i) {
            ...
            emit SKV_SessionKeyPauseToggled(sessionKey.pub, address(SCW), expectedState);
            ...
            assertEq(
                SESSION_KEY_VALIDATOR.isSessionLive(sessionKey.pub),
                expectedState,
    ...
    function testFuzz_getSessionKeysByWallet(uint8 _keyCount) public validatorInstalled {
        ...
        for (uint8 i; i < _keyCount; ++i) {
            ...
            // Set up the original session key
            (SessionData memory sd, Permission[] memory perms) = _getSessionKeyAndPermissions(newSessionKey);
            SESSION_KEY_VALIDATOR.enableSessionKey(sd, perms);
        }
        // Retrieve the session keys associated with the wallet
        address[] memory retrievedKeys = SESSION_KEY_VALIDATOR.getSessionKeysByWallet();
        // Verify the number of retrieved session keys
        assertEq(retrievedKeys.length, _keyCount, "Number of retrieved session keys should match the expected count");
        // Verify each retrieved session key
        for (uint8 i; i < retrievedKeys.length; ++i) {
            assertEq(
                retrievedKeys[i],
                sessionKeys[i],
                string(abi.encodePacked("Session key at index ", i, " should match the expected session key address"))
            );
        }
        vm.stopPrank();
    }

    function testFuzz_getSessionKeyPermissions(uint8 _numPermissions) public validatorInstalled {
        ...
        ParamCondition[][] memory _paramConditions = new ParamCondition[][](_numPermissions);
        ...
        for (uint8 i; i < _numPermissions; ++i) {
            ...
            conditions[0] = ParamCondition({
                ...
                value: keccak256(abi.encodePacked(i, block.timestamp, msg.sender))
            });
            _paramConditions[i] = conditions;
        }
        // Set up the session key with the generated parameters
        SessionData memory sd =
            SessionData({sessionKey: sessionKey.pub, validAfter: validAfter, validUntil: validUntil, live: true});
        ...
        assertEq(permissions.length, _numPermissions, "Number of permissions should match");
        // Verify each permission
        for (uint8 i; i < _numPermissions; ++i) {
            assertEq(permissions[i].target, _targets[i], "Target should match");
            assertEq(permissions[i].selector, _selectors[i], "Selector should match");
            assertEq(permissions[i].payableLimit, _payableLimits[i], "Payable limit should match");
            assertEq(permissions[i].uses, _uses[i], "Uses should match");
            assertEq(permissions[i].paramConditions.length, 1, "Number of param conditions should be 1");
            assertEq(permissions[i].paramConditions[0].offset, i + 2, "Offset should match");
            assertEq(
    ...
    function testFuzz_addPermission(
        ...
        uint256 initialPermissionCount = SESSION_KEY_VALIDATOR.getSessionKeyPermissions(sessionKey.pub).length;
        // Add the new permission
        ComparisonRule rule = ComparisonRule(_rule);
        ParamCondition[] memory paramConditions = new ParamCondition[](1);
        paramConditions[0] = ParamCondition({offset: _offset, rule: rule, value: _value});
        SESSION_KEY_VALIDATOR.addPermission(
            sessionKey.pub,
            Permission({
                target: _target,
                selector: _selector,
        ...
        Permission[] memory updatedPermissions = SESSION_KEY_VALIDATOR.getSessionKeyPermissions(sessionKey.pub);
        // Verify the number of permissions has increased
        assertEq(updatedPermissions.length, initialPermissionCount + 1, "Number of permissions should increase by 1");
        // Verify the new permission details
        Permission memory newPermission = updatedPermissions[initialPermissionCount];
        assertEq(newPermission.target, _target, "New permission target should match");
        assertEq(newPermission.selector, _selector, "New permission selector should match");
        assertEq(newPermission.payableLimit, _payableLimit, "New permission payable limit should match");
        assertEq(newPermission.uses, tenUses, "New permission payable limit should match");
        assertEq(newPermission.paramConditions.length, 1, "New permission should have one param condition");
        assertEq(newPermission.paramConditions[0].offset, _offset, "New permission offset should match");
        assertEq(uint8(newPermission.paramConditions[0].rule), _rule, "New permission rule should match");
        assertEq(newPermission.paramConditions[0].value, _value, "New permission value should match");
    }

    function testFuzz_removePermission(uint8 _permissionIndexToRemove) public validatorInstalled {
        // Set up a session key with initial permissions
        (SessionData memory sd, Permission[] memory perms) = _getSessionKeyAndPermissions(sessionKey);
        SESSION_KEY_VALIDATOR.enableSessionKey(sd, perms);
        // Get the initial number of permissions
        ...
        _permissionIndexToRemove = uint8(bound(_permissionIndexToRemove, 0, initialPermissionCount - 1));
        // Remove the permission at the fuzzed index
        SESSION_KEY_VALIDATOR.removePermission(sessionKey.pub, _permissionIndexToRemove);
        // Retrieve updated session key permissions
        Permission[] memory updatedPermissions = SESSION_KEY_VALIDATOR.getSessionKeyPermissions(sessionKey.pub);
        // Verify the number of permissions has decreased by 1
        assertEq(updatedPermissions.length, initialPermissionCount - 1, "Number of permissions should decrease by 1");
        // Verify that the removed permission is no longer present
        for (uint256 i; i < updatedPermissions.length; ++i) {
            assertTrue(
                i != _permissionIndexToRemove || updatedPermissions[i].target != perms[_permissionIndexToRemove].target
                    || updatedPermissions[i].selector != perms[_permissionIndexToRemove].selector,
    ...
    function testFuzz_modifyPermission(
        ...
        vm.assume(_newPayableLimit != 0 && _newSelector != bytes4(0) && _newUses != 0);
        vm.assume(_paramRule <= uint8(type(ComparisonRule).max));
        (SessionData memory sd, Permission[] memory perms) = _getSessionKeyAndPermissions(sessionKey);
        SESSION_KEY_VALIDATOR.enableSessionKey(sd, perms);
        ParamCondition[] memory newConditions = new ParamCondition[](1);
        newConditions[0] = ParamCondition({offset: _paramOffset, rule: ComparisonRule(_paramRule), value: _paramValue});
        SESSION_KEY_VALIDATOR.modifyPermission(
            sessionKey.pub, 0, _newTarget, _newSelector, _newPayableLimit, _newUses, newConditions
        );
        ...
        assertEq(modifiedPerms[0].selector, _newSelector, "selector doesnt match");
    ...
    function testFuzz_modifyPermission_completeAndPartialModification(
        ...
        address targetToUse = (_modificationMask & 1) != 0 ? _newTarget : originalPerm.target;
        ...
        ParamCondition[] memory conditionsToUse =
            (_modificationMask & 16) != 0 ? newConditions : new ParamCondition[](0);

        SESSION_KEY_VALIDATOR.modifyPermission(
            sessionKey.pub, 0, targetToUse, selectorToUse, payableLimitToUse, usesToUse, conditionsToUse
        );
        Permission[] memory modifiedPerms = SESSION_KEY_VALIDATOR.getSessionKeyPermissions(sessionKey.pub);
        assertEq(modifiedPerms[0].target, targetToUse, "target doesn't match");
        assertEq(
            modifiedPerms[0].selector,
            (_modificationMask & 2) != 0 ? selectorToUse : originalPerm.selector,
            "selector doesn't match"
        );
        assertEq(
            modifiedPerms[0].payableLimit,
            (_modificationMask & 4) != 0 ? payableLimitToUse : originalPerm.payableLimit,
            "payableLimit doesn't match"
        );
        assertEq(
            modifiedPerms[0].uses, (_modificationMask & 8) != 0 ? usesToUse : originalPerm.uses, "uses doesn't match"
        );
        if ((_modificationMask & 16) != 0) {
            assertEq(modifiedPerms[0].paramConditions.length, 1, "paramConditions length doesn't match");
            assertEq(modifiedPerms[0].paramConditions[0].offset, _paramOffset, "paramConditions offset doesn't match");
            assertEq(uint8(modifiedPerms[0].paramConditions[0].rule), _paramRule, "paramConditions rule doesn't match");
            assertEq(modifiedPerms[0].paramConditions[0].value, _paramValue, "paramConditions value doesn't match");
        } else {
            assertEq(
    ...
    function testFuzz_rotateSessionKey(
        ...
        vm.assume(_newSessionKey != address(0) && _newSessionKey != sessionKey.pub);
        _validAfter = uint48(bound(_validAfter, block.timestamp, block.timestamp + 365 days));
        _validUntil = uint48(bound(_validUntil, _validAfter + 1 days, _validAfter + 365 days));
        _uses = bound(_uses, 1, 1000);
        _paramValue = bound(_paramValue, 0, type(uint256).max);
        SessionData memory newSd =
            SessionData({sessionKey: _newSessionKey, validAfter: _validAfter, validUntil: _validUntil, live: _live});
        ParamCondition[] memory newConditions = new ParamCondition[](1);
        newConditions[0] =
            ParamCondition({offset: 4, rule: ComparisonRule.GREATER_THAN_OR_EQUAL, value: bytes32(_paramValue)});
        ...
        assertTrue(
            SESSION_KEY_VALIDATOR.getSessionKeyData(_newSessionKey).validUntil > 0,
        ...
        SessionData memory rotatedData = SESSION_KEY_VALIDATOR.getSessionKeyData(_newSessionKey);
        assertEq(rotatedData.sessionKey, _newSessionKey, "Session key address should match");
        assertEq(rotatedData.validAfter, _validAfter, "Valid after should match");
        assertEq(rotatedData.validUntil, _validUntil, "Valid until should match");
        assertEq(rotatedData.live, true, "Live status should be true after rotation");
        // Verify new permissions
        Permission[] memory rotatedPerms = SESSION_KEY_VALIDATOR.getSessionKeyPermissions(_newSessionKey);
        assertEq(rotatedPerms.length, 1, "Should have one permission");
        assertEq(rotatedPerms[0].target, address(counter2), "Target should match");
        assertEq(rotatedPerms[0].selector, TestCounter.changeCount.selector, "Selector should match");
        assertEq(rotatedPerms[0].payableLimit, 0, "Payable limit should match");
        assertEq(rotatedPerms[0].uses, _uses, "Uses should match");
        assertEq(rotatedPerms[0].paramConditions.length, 1, "Should have one param condition");
        assertEq(rotatedPerms[0].paramConditions[0].offset, 4, "Offset should match");
        assertEq(
            uint8(rotatedPerms[0].paramConditions[0].rule),
            uint8(ComparisonRule.GREATER_THAN_OR_EQUAL),
            "Rule should match"
        );
        assertEq(rotatedPerms[0].paramConditions[0].value, bytes32(_paramValue), "Value should match");
        vm.stopPrank();
    }
    ...
    function testFuzz_updateUses(uint256 _uses) public validatorInstalled {
        ...
        SESSION_KEY_VALIDATOR.updateUses(sessionKey.pub, 0, _uses);
        // Verify that the number of uses has been updated correctly
        assertEq(
            SESSION_KEY_VALIDATOR.getSessionKeyPermissions(sessionKey.pub)[0].uses, _uses, "Uses should be updated"
        );
        vm.stopPrank();
    }

    function testFuzz_updateValidUntil(uint48 initialValidUntil, uint48 newValidUntil) public validatorInstalled {
        ...
        assertEq(updatedData.validUntil, newValidUntil, "ValidUntil should be updated to new value");
    }

    function testFuzz_executeSingle(address _testAddress, uint256 _testUint, bool _testBool)
        public
        validatorInstalled
    {
        ...
        paramConditions[0] =
            ParamCondition({offset: 4, rule: ComparisonRule.EQUAL, value: bytes32(uint256(uint160(_testAddress)))});
        paramConditions[1] =
            ParamCondition({offset: 36, rule: ComparisonRule.LESS_THAN_OR_EQUAL, value: bytes32(_testUint)});
        ...
        bytes memory callData =
            abi.encodeWithSelector(TestCounter.multiTypeCall.selector, _testAddress, _testUint, _testBool);
    ...
    function testFuzz_executeSingle_Native(uint256 _transferAmount, uint256 _payableLimit) public validatorInstalled {
        ...
        perms[0].paramConditions[0] = ParamCondition({offset: 0, rule: ComparisonRule.NOT_EQUAL, value: 0});
        SESSION_KEY_VALIDATOR.enableSessionKey(sd, perms);
        // Create an array of execution validations
        ExecutionValidation[] memory evs = new ExecutionValidation[](1);
        evs[0] = _getExecutionValidation(uint48(1), uint48(3));
        PackedUserOperation memory op =
            _setupSingleUserOp(address(SESSION_KEY_VALIDATOR), bob.pub, _transferAmount, hex"", evs, sessionKey);
        // Execute the user operation
        _executeUserOp(op);
        ...
        assertEq(bob.pub.balance, 100 ether + _transferAmount, "Receiver balance should match transferred amount");
    ...
    function testFuzz_executeBatch(address _testAddress, uint256 _testUint, bool _testBool, uint256 _countValue)
        public
        validatorInstalled
    {
        ...
        perms[0].paramConditions[0] =
            ParamCondition({offset: 4, rule: ComparisonRule.EQUAL, value: bytes32(uint256(uint160(_testAddress)))});
        perms[0].paramConditions[1] =
            ParamCondition({offset: 36, rule: ComparisonRule.LESS_THAN_OR_EQUAL, value: bytes32(_testUint)});
        ...
        bytes memory callData1 =
            abi.encodeWithSelector(TestCounter.multiTypeCall.selector, _testAddress, _testUint, _testBool);
        bytes memory callData2 = abi.encodeWithSelector(TestCounter.changeCount.selector, _countValue);
