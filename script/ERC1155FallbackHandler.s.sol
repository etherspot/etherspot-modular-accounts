...
contract ERC1155FallbackHandlerScript is Script {
    ...
    function run() external {
        ...
        /*//////////////////////////////////////////////////////////////
                        Deploy ERC1155FallbackHandler
        //////////////////////////////////////////////////////////////*/
        console2.log("Deploying ERC1155FallbackHandler implementation...");
        if (EXPECTED_ERC1155_FALLBACK_HANDLER.code.length == 0) {
            ERC1155FallbackHandler erc1155Fallback = new ERC1155FallbackHandler{salt: SALT}();
            if (address(erc1155Fallback) != EXPECTED_ERC1155_FALLBACK_HANDLER) {
                revert("Unexpected ERC1155 fallback handler address!!!");
            } else {
                console2.log("ERC1155 fallback handler deployed at address", address(erc1155Fallback));
                // bytes memory implCode = address(erc1155Fallback).code;
                // console2.logBytes(implCode);
            }
        } else {
            console2.log("ERC1155 fallback handler already deployed at address", EXPECTED_ERC1155_FALLBACK_HANDLER);
        }
