...
contract ERC20SessionKeyValidatorScript is Script {
    ...
    address public constant EXPECTED_ERC20_SESSION_KEY_VALIDATOR = 0x60Da6Cc14d817a88DC354d6dB6314DCD41b7aA54;
    ...
    function run() external {
        ...
        if (EXPECTED_ERC20_SESSION_KEY_VALIDATOR.code.length == 0) {
            ERC20SessionKeyValidator erc20SessionKeyValidator = new ERC20SessionKeyValidator{salt: SALT}();
            if (address(erc20SessionKeyValidator) != EXPECTED_ERC20_SESSION_KEY_VALIDATOR) {
                revert("Unexpected wallet implementation address!!!");
            } else {
                console2.log("ERC20SessionKeyValidator deployed at address", address(erc20SessionKeyValidator));
            }
        } else {
            console2.log("Already deployed at address", EXPECTED_ERC20_SESSION_KEY_VALIDATOR);
        }
